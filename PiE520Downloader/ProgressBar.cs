    ...
    public class ProgressBar : IDisposable, IProgress<double>
    {
        ...
        private void TimerHandler(object state)
        {
            lock (_timer)
            {
                ...
                float percent = (float) (_currentProgress * 100);
                string text =
                    $"[{new string('#', progressBlockCount)}{new string('-', BlockCount - progressBlockCount)}] {percent,3:F}% {Animation[_animationIndex++ % Animation.Length]}";
                UpdateText(text);
